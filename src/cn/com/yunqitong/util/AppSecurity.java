...
public class AppSecurity {
	...
	public static Map<String,String> valid(String timestamp,String authenticator ){
		...
		Date date=new Date();
		userDate.setTime(userDate.getTime()+5*60*1000);
		
		String format1 = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(userDate);
		System.out.println(" 加 5 "+format1);
		
		String format = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(date);
		System.out.println("现在时间 "+format);
		
		Date currentDate = DateHelper.transferFormat2Date(format);
		int comResult = userDate.compareTo(currentDate);
		if(comResult==1){
			log.info("timestamp is not expired ....");
		}else{
			map.put(AppSecurity.code, "20025");
			map.put(AppSecurity.msg, "此次请求已过期,请重新发送请求");
			log.info("时间戳已失效....");
			return map;
		}
